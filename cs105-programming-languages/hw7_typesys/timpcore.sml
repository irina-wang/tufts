...
(* function [[ty]], checks type of expression, given $\itenvs$ 347b *)
fun typeof (e, globals, functions, formals) =
  let 
      fun get_array_type (ARRAYTY(t)) = t
        | get_array_type _            = raise TypeError ("Invalid array type")
...
(* function [[ty]], checks type of expression, given $\itenvs$ ((prototype)) 357 *)
        | ty (AAT (a, i)) = 
            let val tau_1 = get_array_type (ty a)
                val tau_2 = ty i
            in if eqType (ty i, INTTY)
                    then tau_1 
                    else raise TypeError ("Index i not an integer")
            end
        | ty (APUT (a, i, e)) = 
            let val tau_1 = get_array_type (ty a)
                val tau_2 = ty i
                val tau_3 = ty e
            in if eqType(tau_2, INTTY) andalso eqType(tau_1, tau_3)
                    then tau_1 
                    else raise TypeError ("Index i not an integer")
            end
        | ty (AMAKE (len, init)) = 
            let val tau_1 = ty len
                val tau_2 = ty init
            in if eqType(tau_1, INTTY)
                    then (ARRAYTY(tau_2)) 
                    else raise TypeError ("length is not an int")
            end
        | ty (ASIZE a)           = 
            let val tau_1 = get_array_type (ty a)
            in INTTY
