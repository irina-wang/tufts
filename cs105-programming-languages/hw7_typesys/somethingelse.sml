...
fun typeof (e, kindEnv, typeEnv) = 
  let fun ty (LITERAL x) =
      (case x  
          ...
          |  PAIR (a, b) => 
              let val tau1 = ty (LITERAL a)
                  val tau2 = ty (LITERAL b)
              in  if eqType (listtype tau1, tau2) then
                    tau2
                  else
                    raise TypeError
                      "Literal Pair contains mixed types"
              end
          | _ => raise TypeError "Invalid type for Literals")
        | ty (IFX (e1, e2, e3)) = 
              let val tau1  = ty e1 
                  val tau2  = ty e2 
                  val tau3  = ty e3
        ...
        | ty (SET (x, e)) = 
            let val tau = ty e
            in  if eqType(tau, find(x, typeEnv)) then tau 
                else 
                  raise TypeError ("Set variable " ^ x ^ " of type " ^
                                typeString tau ^ " to value of type " ^
        ...
        | ty (LETX (LETSTAR, bs, e)) = 
            (case bs of
                []         => ty e
                | (x::xs)  => ty (LETX (LET, [x], LETX (LETSTAR, xs, e))))
        | ty (LETRECX (bs, e)) = 
  ...
  let fun addKind ((name, kind), kinds) =
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            ...
                            (* primitive functions for \tuscheme\ [[::]] 391c *)
                             ...
                             ("!", unaryOp (fn (ARRAY a) => Array.sub (a, 0)
                                              |  v   => raise BugInTypeChecking 
                                     "! applied to non-list " ^ valueString v)
                                     , FORALL (["'a"], FUNTY ([reftype tvA], 
                                                                      tvA))) ::
                              
                             (":=", binaryOp (fn (ARRAY a, v) => (Array.update (a, 0, v); unitVal)
                                                       |  _   => raise RuntimeError  
                                     ":= applied to non-list" ^  valueString a 
                                                "or non-value" ^ valueString v)
...
val initialBasis =
  let val predefined_included = false
