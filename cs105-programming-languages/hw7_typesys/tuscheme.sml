...
fun typeof (e, kindEnv, typeEnv) = 
  let fun ty (LITERAL v) = 
          let fun literal (LITERAL (NUM v))       = inttype
                | literal (LITERAL (BOOLV v))     = booltype
                | literal (LITERAL (SYM v))       = symtype
                | literal (LITERAL NIL)           = FORALL(["'a"], listtype tvA)
                | literal (LITERAL (PAIR (v, NIL))) = listtype (ty (LITERAL v))
                | literal (LITERAL (PAIR (v, v')))  = 
                    let val tau      = ty (LITERAL v)
                        val tau_list = ty (LITERAL v')
                    in  if eqType (tau_list, listtype tau) then 
                          tau_list 
                        else 
                          raise TypeError "PAIR not of the same type"
                    end
                | literal _ = raise TypeError "LITERAL - TypeError"
          in literal (LITERAL v)
          end 
        | ty (IFX (e1, e2, e3)) = 
              let val tau1  = ty e1 
                  val tau2  = ty e2 
        ...
        | ty (SET (x, e)) = 
            let val tau1 = find (x, typeEnv)
                val tau2 = ty e
            in  if eqType(tau1, tau2) then tau1
                else 
                  raise TypeError ("Set variable " ^ x ^ " of type " ^
                                typeString tau1 ^ " to value of type " ^
                                typeString (typeof (e, kindEnv, typeEnv)))
            end
        | ty (BEGIN [])  = unittype 
        | ty (BEGIN es) = 
        ...
        | ty (VAR x) = find (x, typeEnv)        
        | ty (APPLY (e, es)) = 
            let val expType = case ty e 
                                of FUNTY(ts, t) => (ts, t)
                                 | _ => raise TypeError "Error expression-type"
                val types    = map ty es  
            in  if eqTypes(types, fst expType) then 
                  snd expType 
                else 
                  raise TypeError "Apply - Mismatched type/number of elements"
            end
        | ty (LETX (LET, bs, e)) = 
            let val types   = map ty (map snd bs)
                val newTypeEnv = bindList(map fst bs, types, typeEnv)
            in typeof (e, kindEnv, newTypeEnv)
            end 
        | ty (LETX (LETSTAR, [], e)) = ty e
        | ty (LETX (LETSTAR, (x::xs), e)) = 
              ty (LETX (LET, [x], LETX (LETSTAR, xs, e)))
        | ty (LAMBDA (args, e)) = 
            let val types  = map snd args
                val newTypeEnv = bindList(map fst args, types, typeEnv)
            in  if List.all (fn(t) => eqKind(kindof(t, kindEnv), TYPE)) types
                then 
                  FUNTY(types, typeof (e, kindEnv, newTypeEnv))
                else 
                  raise TypeError "Lambda - A name doesn't have kind *"
            end 
        | ty (LETRECX (bs, e)) = 
            let val types = map snd (map fst bs) 
                val names = map fst (map fst bs)
                val exps  = map snd bs
                val exps_types = ListPair.zip (exps, types)
                val newTypeEnv = bindList(names, types, typeEnv)
            in if List.all (fn(t) => eqKind(kindof(t, kindEnv), TYPE)) types
                  andalso 
                  List.all (fn(t) => eqType(typeof(fst t, kindEnv, 
                                            newTypeEnv), snd t)) exps_types
                  andalso 
                  List.all (fn(e) => case e of LAMBDA _ => true
                                              | _       => false) exps
                  then typeof(e, kindEnv, newTypeEnv)
               else 
                  raise TypeError 
                  "LETREC - A name doesn't have kind * or an exp isn't LAMBDA"
            end
        | ty (TYLAMBDA (alphas, e)) =  
            let val newKindEnv = foldl (fn(n, env) => bind(n, TYPE, env)) 
                                        kindEnv alphas
                val ftvGamma   = freetyvarsGamma typeEnv
            in  if not (List.exists
                       (fn(a) => List.exists(fn(v) => a = v) ftvGamma) alphas)
                then FORALL(alphas, typeof(e, newKindEnv, typeEnv))
                else 
                  raise TypeError 
                  "TYLAMBDA - A name is free variable or isn't of kind *"
            end 
        | ty (TYAPPLY (e, taus)) = 
            let val exp_tau = case ty e 
                                of FORALL(alphas, tau) =>  FORALL(alphas, tau)
                                  | _ => raise TypeError "Incorrect exp type"
            in  if List.all (fn(t) => eqKind(kindof(t, kindEnv), TYPE)) taus
                then instantiate(exp_tau, taus, kindEnv)
                else 
                  raise TypeError "TYAPPLY - A type is not *"
            end
  in ty e
  end

(*VAL, VALREC, EXP, and DEFINE.*)
fun typdef (def, kindEnv, typeEnv) = 
  let fun ty (VAL (x, e)) = 
            let val tau = typeof(e, kindEnv, typeEnv)
            in  (bind(x, tau, typeEnv), typeString tau)
            end
        | ty (VALREC (x, tau, e)) = 
            let val kind = kindof(tau, kindEnv)
                val newTypeEnv = bind(x, tau, typeEnv)
                val expLambda = case e of LAMBDA _ => true
                                              | _  => false
            in  if eqKind(kind, TYPE) andalso 
                  eqType(typeof(e, kindEnv, newTypeEnv), tau) andalso 
                  expLambda 
                  then (newTypeEnv, typeString tau)
                else 
                  raise TypeError 
                  "VALREC - A name doesn't have type * or an exp isn't LAMBDA"
            end
        | ty (EXP e) = typdef (VAL("it", e), kindEnv, typeEnv)
        | ty (DEFINE (f, tau, (bs, e))) =
            let val types = map snd bs 
            in typdef (VALREC(f, FUNTY(types, tau), LAMBDA(bs, e)), 
                        kindEnv, typeEnv)
            end 
  in ty def 
  end
  ...
  let fun addKind ((name, kind), kinds) =
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            ...
                            (* primitive functions for \tuscheme\ [[::]] 391c *)
                             ...
                             ("ref" , unaryOp (fn (v)=> singletonArray v)
                                    , FORALL (["'a"], FUNTY ([tvA], 
                                                               reftype tvA))) ::
                             ...
                             ("!", unaryOp (fn (ARRAY a) 
                                     ...
                                     "! should have had an array" )
                                     , FORALL (["'a"], FUNTY ([reftype tvA], 
                                                                       tvA))) ::
                              
                             (":=", binaryOp (fn (ARRAY a, v) 
                                           => (Array.update (a, 0, v); unitVal)
                                    |  _   => raise RuntimeError  
                                     ":= applied to non-list")
                                    , FORALL (["'a"], FUNTY ([reftype tvA, tvA],
                                                                  unittype))) ::
