...
fun freeIn (LITERAL l)        _ = false 
  | freeIn (VAR v)            n = (v = n)
  | freeIn (SET (x, e))       n = (x = n) orelse (freeIn e n) 
  | freeIn (IFX (e1, e2, e3)) n     = (freeIn e1 n) orelse (freeIn e2 n) 
                                                orelse (freeIn e3 n)
  | freeIn (WHILEX  (e1, e2)) n     = (freeIn e1 n) orelse (freeIn e2 n)
  | freeIn (BEGIN   es)       n     =  List.exists (fn ei => freeIn ei n) es
  | freeIn (APPLY  (e, es))   n     = (freeIn e n) orelse 
                                      List.exists (fn ei => freeIn ei n) es
  | freeIn (LAMBDA (args, e)) n     = (freeIn e n) andalso (not (member n args))
  | freeIn (LETX (LET, pairs, e)) n = List.exists (fn pi => freeIn (snd pi) n) pairs
                                      orelse  ((not (member n (map fst pairs))) andalso (freeIn e n))
  | freeIn (LETX (LETREC, pairs, e)) n = (not (member n (map fst pairs))) andalso 
                                         ((List.exists (fn pi => freeIn (snd pi) n) pairs) orelse (freeIn e n))
  | freeIn (LETX (LETSTAR, [], e)) n = (freeIn e n)
  | freeIn (LETX (LETSTAR, b::bs, e)) n = freeIn (LETX (LET, b::[], LETX (LETSTAR, bs, e))) n

(* Unit Tests *)
  (* LITERAL *)
        (* val () = Unit.checkAssert "freeIn - LIT - false"
                (fn () => not (freeIn (LITERAL (NUM 1)) "1"))  

        val () = Unit.checkAssert "freeIn - LIT w/ rand name - false" *)
                (fn () => not (freeIn (LITERAL (NUM 1)) "true"))  
  (* VAR *)
        (* val () = Unit.checkAssert "freeIn - VAL - true"
                (fn () => freeIn ((VAL "hi") "hi")) 

        val () = Unit.checkAssert "freeIn - VAL - true"
                (fn () => not (freeIn (VAL "hi") "bye"))  *)
  (* SET *)
        (* val () = Unit.checkAssert "freeIn - VAL - true"
                (fn () => freeIn ((SET (SYMBOL x) 2) "x"))   *)
        (* val () = Unit.checkAssert "freeIn - VAL - false"
                (fn () => not (freeIn (name "hi")(VAL "hi") "bye"))  *)
...
(*    > takes a pair consisting of a LAMBDA body and an environment 
      > returns a better pair containing                                  *)
fun improve (names, envr) = 
                    (names, List.filter (fn (x, _) => freeIn (LAMBDA names) x) envr)


(* fun ev (LAMBDA (xs, e)) = improve (xs, rho) *)



(* • The third part is to use improve in the evaluation case for LAMBDA, which appears in the book on page 308d. You simply apply improve to the pair that is already there, so your improved interpreter looks like this: *)