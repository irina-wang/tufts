...
fun solve TRIVIAL = idsubst
  ...
  | solve (TYVAR a ~ TYVAR b) = 
    if a = b then idsubst 
      else a |--> TYVAR b
  | solve (TYVAR a ~ TYCON tau) = a |--> TYCON tau
  | solve (TYCON tau ~ TYVAR a) = a |--> TYCON tau
  | solve (TYVAR a ~ CONAPP tau) = 
      if not (member a (freetyvars (CONAPP tau)))
          then a |--> CONAPP tau
      else unsatisfiableEquality(TYVAR a, CONAPP tau)
  | solve (CONAPP tau ~ TYVAR a) = solve ( TYVAR a ~ CONAPP tau)
  | solve (TYCON t1 ~ TYCON t2) =
      if t1 = t2
          then idsubst 
      else unsatisfiableEquality(TYCON t1, TYCON t2)
  | solve ( TYCON t1 ~ CONAPP t2) = unsatisfiableEquality(TYCON t1, CONAPP t2)
  | solve ( CONAPP t1 ~ TYCON t2) = unsatisfiableEquality(CONAPP t1, TYCON t2)
  | solve ( CONAPP (TYCON ty1, tys1) ~ CONAPP (TYCON ty2, tys2)) = 
      let  fun unsolvable () =  unsatisfiableEquality( CONAPP (TYCON ty1, tys1),
                                                   CONAPP (TYCON ty2, tys2))
           fun singleconst (t1, t2) = t1 ~ t2
           fun remaincons (ls1, ls2) = List.map singleconst (ListPair.zipEq 
                                                                  (ls1, ls2))
                      handle UnequalLengths =>  unsolvable()
      in if ty1 = ty2 
            then solve (conjoinConstraints (remaincons (tys1, tys2)))
       else unsolvable()
      end
  | solve _ = raise TypeError ""
...
fun solutionEquivalentTo (c, theta) = eqsubst (solve c, theta)
    ...
    (* Unit tests *)
        (********* TRIVIAL ********)
        val () = Unit.checkAssert ("trivial is solved by the " ^ 
                "identity substitution")
                (fn () => solutionEquivalentTo (TRIVIAL, idsubst))

        (********* C1 /\ C2 *********)
        val () = Unit.checkAssert ("'a ~ 'b /\\ 'b ~ bool" ^
        " is solved by 'a |--> 'bool, 'b |--> bool")
            (fn () => solutionEquivalentTo (
                TYVAR "'a" ~ TYVAR "'b" /\ TYVAR "'b" ~ TYCON "bool",
                compose ("'b" |--> booltype, "'a" |--> booltype)))
        val () = Unit.checkAssert 
              "'a ~ inttype /\\ 'a ~ booltype has no solution"
              (fn () => hasNoSolution 
              (alpha ~ inttype /\ alpha ~ booltype))
        ...
        val () = Unit.checkAssert ("bool ~ bool is solved by the " ^ 
                "identity substitution")
        ...
        val () = Unit.checkAssert 
              ...
              (fn () => solutionEquivalentTo 
                            (listtype (TYVAR "'a") ~ listtype (TYVAR "'b"), 
                              "'a" |--> TYVAR "'b"))
        val () = Unit.checkAssert 
          "(pair 'a bool) ~ (pair 'b 'c) is solved by 'a |--> 'b,'c |--> bool"
              (fn () => solutionEquivalentTo 
              (pairtype (alpha, booltype) ~ pairtype (beta, TYVAR "'c"),
               compose ("a" |--> beta, "'c" |--> booltype)))
        val () = Unit.checkAssert 
          "(pair 'a bool) ~ (pair 'b 'a) is solved by 'a |--> bool,'b |--> bool"
              (fn () => solutionEquivalentTo 
              (pairtype (alpha, booltype) ~ pairtype (beta, alpha),
               compose ("a" |--> booltype, "b" |--> booltype)))
        val () = Unit.checkAssert 
          "(pair 'a 'a) ~ (pair bool int) has no solution"
              (fn () => hasNoSolution
                  (pairtype (alpha, alpha) ~ pairtype (booltype, inttype)))
        val () = Unit.checkAssert 
              "(pair 'a bool) ~ (pair 'b) has unequal length "
              (fn () => hasNoSolution 
              (pairtype (alpha, inttype) ~ CONAPP (TYCON "pair", [beta])))
        val () = Unit.checkAssert 
              "(pair 'a int) ~ (pair 'a int) is solved by idsubst "
              (fn () => solutionEquivalentTo 
                        (pairtype (alpha, inttype) ~ pairtype (alpha, inttype),
                         idsubst))

        (********* TYVAR & TYCON (has solution) *********)
        (* TYVAR ~ TYCON *)
        ...
        (********* TYCON & CONAPP (NoSolution) *********)
        (* TYCON ~ CONAPP *)
        val () = Unit.checkAssert "int ~ (list int) is unsolvable"
                (fn () => hasNoSolution (inttype ~ (listtype inttype)))
        val () = Unit.checkAssert "int ~ (list bool) is unsolvable"
                (fn () => hasNoSolution (inttype ~ (listtype booltype)))
        (* CONAPP ~ TYCON *)
        val () = Unit.checkAssert "(list bool) ~ bool is unsolvable"
                (fn () => hasNoSolution ((listtype booltype) ~ booltype))


        (********* TYVAR vs CONAPP ********)
        (* TYVAR ~ CONAPP *)
        val () = Unit.checkAssert "'a ~ (list 'a) is unsolvable"
                (fn () => hasNoSolution (alpha ~ (listtype alpha)))
        val () = Unit.checkAssert "'a ~ (list 'b) is solved by 'a |-> (b list)"
                (fn () => solutionEquivalentTo (listtype beta ~ alpha,
                      "a" |--> listtype beta))
        (* CONAPP ~ TYVAR *)
        val () = Unit.checkAssert "'(list 'b) ~ a is solved by 'a |-> (b list)"
                (fn () => solutionEquivalentTo (alpha ~ listtype beta,
                      "a" |--> listtype beta))
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 448e *)
      fun literal (SYM s)         = (symtype, TRIVIAL)
        ...
        | literal (PAIR (v, v'))  = 
                    let val (t1, c1) = literal v
                        val (t2, c2) = case v' of NIL => (listtype t1, TRIVIAL)
                                                | _   => literal v'
                      in (t2, c1 /\ c2 /\ listtype t1 ~ t2)
                    end
        | literal _ = raise TypeError "Invalid literal types" 
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (BEGIN es)                = 
            (case es 
              of [] => (unittype, TRIVIAL)
               | _ => 
                  let val (ts, cs) = typesof(es, Gamma)
                      val rettype = List.last ts
                    in (rettype, cs)
                  end) 
        | ty (LAMBDA (formals, body))  = 
            let
                val newGamma = foldl (fn (x, env) => bindtyscheme 
                            (x, FORALL([x],freshtyvar()), env)) Gamma formals
                val (tau, c)  = typeof(body, newGamma)
                val newformals = List.map (fn (n) => freshInstance
                                          (findtyscheme (n, newGamma))) formals
            in (funtype (newformals, tau), c)
            end
        | ty (LETX (LET, bs, body))    = 
            let 
                val (xs, es) = ListPair.unzip bs
                val (tau, c) = typesof (es, Gamma)
                val theta    = solve c 
                val domC' = inter (dom theta, freetyvarsGamma Gamma)
                val c' = conjoinConstraints
                            (map (fn a => 
                                        TYVAR a ~ (varsubst theta a)) domC')
                val sigma  = map (fn t => generalize (tysubst theta t,
                                            union (freetyvarsGamma Gamma, 
                                            freetyvarsConstraint c'))) tau
                val newformals = ListPair.zipEq (xs, sigma)
                val newGamma = foldl (fn ((x, env), gamma) => 
                                bindtyscheme (x, env, gamma)) Gamma newformals
                val (t, c)  = typeof(body, newGamma)
            in (t, c' /\ c)
            end
        | ty (LETX (LETREC, bs, body)) = 
            let
                val (ns, es)  = ListPair.unzip bs
                val tyvars  = map (fn x => freshtyvar x) ns
                val tyschemes  = map (fn x => FORALL([], x)) tyvars 
                val newformals = ListPair.zipEq (ns, tyschemes)
                val gamma'    = foldl (fn ((x, env), gamma) => 
                                bindtyscheme (x, env, gamma)) Gamma newformals
                val (ts, c_r) = typesof(es, gamma')
                val c' = conjoinConstraints (map (fn (t, a) => t ~ a)
                                       (ListPair.zipEq (ts, tyvars)))
                val c = c_r /\ c'
                val theta = solve c
                val alphas = inter (dom theta, freetyvarsGamma Gamma)
                val c'' = conjoinConstraints
                            (map (fn a => 
                                        TYVAR a ~ (varsubst theta a)) alphas)
                val sigma = map (fn t => generalize (tysubst theta t, 
                                          union(freetyvarsGamma Gamma, 
                                          freetyvarsConstraint c''))) ts
                val newformals = ListPair.zipEq (ns, sigma)
                val newGamma = foldl (fn ((x, env), gamma) => 
                                bindtyscheme (x, env, gamma)) Gamma newformals
                val (t, c)  = typeof(body, newGamma)
                val checkLambda = List.all (fn e => case e of LAMBDA _ => true
                                              | _ => false) es
            in if checkLambda then (t, c'' /\ c)
                  else raise TypeError "Lecrec expects Lambda in args"
            end
(* type declarations for consistency checking *)
